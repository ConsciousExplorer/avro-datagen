...
* add the validator module to the API reference ([#25](https://github.com/ConsciousExplorer/avro-datagen/issues/25)) ([b39537f](https://github.com/ConsciousExplorer/avro-datagen/commit/b39537f73eb7479e07d2557ba6629574bcec2331))
...
- **`KAFKA_COMPRESSION_TYPE` env var ignored by Streamlit UI** (#13) — the Compression selectbox hardcoded `snappy` and ignored the env var advertised in `.env.sample`. It now reads `KAFKA_COMPRESSION_TYPE` and falls back to `none` when unset or set to an unknown value, mirroring how `KAFKA_LINGER_MS` and `KAFKA_BATCH_SIZE` already work.

### Changed

- Default `compression_type` for `build_producer_config` is now `"none"` (was `"snappy"`), so the programmatic API matches the UI default.
- `.env.sample` documents `KAFKA_COMPRESSION_TYPE=none` as the default.
- Docs: streamlit-ui env-var table now includes a Compression row with valid values; the kafka-producer-app example uses `"none"`.
